...
public abstract class MapSearchRequest extends SearchRequest {
	public MapSearchRequest(LexicalAdminClient client) {
		super(client);
	}
	
	@SuppressWarnings("unchecked")
	public <T extends LexicalResponse> T execute() {
		MapSearchResponse searchResponse;
		Reader reader = null;
		try {
			...
			searchResponse = new MapSearchResponse(true);
			searchResponse.setItems(items);
		}
		catch (Exception e) {
			searchResponse = new MapSearchResponse(false);
			log.error(e);
		}
		finally {
			try {
